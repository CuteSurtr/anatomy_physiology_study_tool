...
import { pharmacology, getPharmacology } from "@/lib/content";
...
export function generateStaticParams() {
  return pharmacology.map((p) => ({ slug: p.slug }));
}
...
export async function generateMetadata({
  ...
  const page = getPharmacology(slug);
  if (!page) return { title: "Not found" };
  const title = `${page.title} - Pharmacology`;
  const description =
    page.summary ??
    `${page.title} - mechanism, indications, side effects, contraindications. Pharmacology reference.`;
  return {
    title,
    description,
...
export default async function PharmacologyPage({
  ...
  const sorted = [...pharmacology].sort(
    (a, b) =>
